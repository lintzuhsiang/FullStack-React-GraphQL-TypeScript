...
import { share as n, merge as a, mergeMap as o, fromArray as s, buffer as u, fromPromise as c, skipUntil as l, map as d, filter as p, empty as f, makeSubject as v } from "wonka";
...
function isFieldAvailableOnType(e, t, r) {
  return _.test(r) || _.test(t) || !!getField(e, t, r);
}
...
function clearDataState() {
  ...
  if ("test" !== process.env.NODE_ENV && !t.defer) {
    ...
    Promise.resolve().then((function() {
      ...
      !function gc() {
        b.gc.forEach((function(e, t, r) {
          if ((b.refCount[e] || 0) > 0) {
            r.delete(e);
            return;
          }
          for (var i in b.refLock) {
            var n = b.refLock[i];
            if ((n[e] || 0) > 0) {
              return;
            }
          ...
          delete b.refCount[e];
          r.delete(e);
          b.records.base.delete(e);
          var a = b.links.base.get(e);
          if (a) {
            b.links.base.delete(e);
            for (var o in a) {
              updateRCForLink(r, b.refCount, a[o], -1);
            }
          }
        }));
      }();
...
function getCurrentOperation() {
  invariant(null !== E, "production" !== process.env.NODE_ENV ? "Invalid Cache call: The cache may only be accessed or mutated duringoperations like write or query, or as part of its resolvers, updaters, or optimistic configs." : "", 2);
  return E;
}
...
function getNode(e, t, r) {
  var i;
  var n = !F && "read" === E && D && b.commutativeKeys.has(D);
  for (var a = 0, o = b.optimisticOrder.length; a < o; a++) {
    var s = b.optimisticOrder[a];
    var u = e.optimistic[s];
    n = n && s !== D;
    if (u && (!n || !b.commutativeKeys.has(s)) && (!F || "write" === E || b.commutativeKeys.has(s)) && void 0 !== (i = u.get(t)) && r in i) {
      return i[r];
    }
  }
...
function extractNodeMapFields(e, t, r, i) {
  ...
  for (var n = 0, a = b.optimisticOrder.length; n < a; n++) {
    var o = i.optimistic[b.optimisticOrder[n]];
    if (void 0 !== o) {
...
function updatePersist(e, t) {
  if (!F && b.storage) {
    b.persist.add(function serializeKeys(e, t) {
...
function readRecord(e, t) {
  ...
  return getNode(b.records, e, t);
}

function readLink(e, t) {
  updateDependencies(e, t);
  return getNode(b.links, e, t);
}

function writeRecord(e, t, r) {
  updateDependencies(e, t);
  updatePersist(e, t);
  setNode(b.records, e, t, r);
}
...
function writeLink(e, t, r) {
  ...
  var o;
  if (D) {
    n = i.refLock[D] || (i.refLock[D] = makeDict());
    a = i.links.optimistic[D];
  } else {
    n = i.refCount;
    a = i.links.base;
    o = i.gc;
...
function writeSelection(e, t, r, i) {
  ...
  while (u = s()) {
    ...
    if (e.optimistic && a) {
      ...
      f = i[p] = ensureData(v(l || {}, e.store, e));
    }
    if (u.selectionSet) {
      if (t && !a) {
        var y = joinKeys(t, d);
        writeLink(t || o, d, writeField(e, getSelectionSet(u), ensureData(f), y));
      } else {
        writeField(e, getSelectionSet(u), ensureData(f));
      }
    } else if (t && !a) {
      writeRecord(t || o, d, null !== f || !getFieldError(e) ? f : void 0);
    }
    if (a) {
...
var L = /^__|PageInfo|(Connection|Edge)$/;
...
function writeField(e, t, r, i) {
  ...
  var l = r.__typename;
  if ("production" !== process.env.NODE_ENV) {
    if (i && !e.store.keys[r.__typename] && null === c && "string" == typeof l && !L.test(l)) {
...
Store.prototype.keyOfEntity = function keyOfEntity(e) {
  if (S.current && e === S.current.parent) {
    return S.current.parentKey;
  }
...
function readSelection(e, t, r, i, n) {
  ...
  while (void 0 !== (l = c())) {
    ...
    if (void 0 !== N && void 0 === l.selectionSet) {
      O = N;
    } else if ("read" === getCurrentOperation() && k && "function" == typeof k[f]) {
      updateContext(e, i, u, s, h, f);
      if (void 0 !== g) {
        i[y] = g;
      }
      O = k[f](i, v || {}, a, e);
      if (l.selectionSet) {
        O = resolveResolverResult(e, u, f, h, getSelectionSet(l), i[y], O);
      }
      if (a.schema && null === O && !isFieldNullable(a.schema, u, f)) {
        return;
      }
    } else if (!l.selectionSet) {
      O = g;
    } else if (void 0 !== N) {
      O = resolveResolverResult(e, u, f, h, getSelectionSet(l), i[y], N);
    } else {
      var _ = readLink(s, m);
      if (void 0 !== _) {
        O = resolveLink(e, _, u, f, getSelectionSet(l), i[y]);
    ...
    e.__internal.path.pop();
    if (void 0 === O && a.schema && isFieldNullable(a.schema, u, f)) {
      p = !0;
      i[y] = null;
    } else if (void 0 === O) {
      return;
...
function resolveResolverResult(e, t, r, i, n, a, o) {
  if (Array.isArray(o)) {
    ...
    for (var l = 0, d = o.length; l < d; l++) {
      ...
      var p = resolveResolverResult(e, t, r, joinKeys(i, "" + l), n, null != a ? a[l] : void 0, o[l]);
      e.__internal.path.pop();
      if (void 0 === p && !u) {
        return;
      } else {
    ...
    var f = a || {};
    return "string" == typeof o ? readSelection(e, o, n, f) : readSelection(e, i, n, f, o);
  } else {
  ...
  return function(t) {
    ...
    function updateCacheWithResult(e, t) {
      ...
      if ("mutation" === r.kind) {
        ...
        reserveLayer(h.data, r.key);
      }
      var o;
      if (e.data) {
        collectPendingOperations(t, write(h, r, e.data, e.error, a).dependencies);
        var s = query(h, r, e.data, e.error, a);
        e.data = s.data;
        if ("query" === r.kind) {
    ...
    return function(e) {
      ...
      var i = n(d(operationResultFromCache)(p((function(e) {
      ...
      var D = n(v(d(prepareForwardedOperation)(a([ O, b ]))));
      var F = d((function(e) {
        var t = new Set;
