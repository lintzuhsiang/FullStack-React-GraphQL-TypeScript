...
function shouldInclude(e, t) {
  ...
  for (var n = 0, a = r.length; n < a; n++) {
    var o = r[n];
    var s = getName(o);
    if (("include" === s || "skip" === s) && o.arguments && o.arguments[0] && "if" === getName(o.arguments[0])) {
      var u = i.valueFromASTUntyped(o.arguments[0].value, t);
      return "include" === s ? !!u : !u;
    }
  }
  return !0;
}
...
function isFieldAvailableOnType(e, t, r) {
  return s.test(r) || s.test(t) || !!getField(e, t, r);
}
...
function clearDataState() {
  ...
  if ("test" !== process.env.NODE_ENV && !t.defer) {
    ...
    Promise.resolve().then((function() {
      ...
      !function gc() {
        c.gc.forEach((function(e, t, r) {
          if ((c.refCount[e] || 0) > 0) {
            r.delete(e);
            return;
          }
          for (var i in c.refLock) {
            var n = c.refLock[i];
            if ((n[e] || 0) > 0) {
              return;
            }
          ...
          delete c.refCount[e];
          r.delete(e);
          c.records.base.delete(e);
          var a = c.links.base.get(e);
          if (a) {
            c.links.base.delete(e);
            for (var o in a) {
              updateRCForLink(r, c.refCount, a[o], -1);
            }
          }
        }));
      }();
...
function getCurrentOperation() {
  invariant(null !== u, "production" !== process.env.NODE_ENV ? "Invalid Cache call: The cache may only be accessed or mutated duringoperations like write or query, or as part of its resolvers, updaters, or optimistic configs." : "", 2);
  return u;
}

function getCurrentDependencies() {
  invariant(null !== l, "production" !== process.env.NODE_ENV ? "Invalid Cache call: The cache may only be accessed or mutated duringoperations like write or query, or as part of its resolvers, updaters, or optimistic configs." : "", 2);
  return l;
}
...
function getNode(e, t, r) {
  var i;
  var n = !f && "read" === u && d && c.commutativeKeys.has(d);
  for (var a = 0, o = c.optimisticOrder.length; a < o; a++) {
    var s = c.optimisticOrder[a];
    var l = e.optimistic[s];
    n = n && s !== d;
    if (l && (!n || !c.commutativeKeys.has(s)) && (!f || "write" === u || c.commutativeKeys.has(s)) && void 0 !== (i = l.get(t)) && r in i) {
      return i[r];
    }
  }
...
function extractNodeMapFields(e, t, r, i) {
  ...
  for (var n = 0, a = c.optimisticOrder.length; n < a; n++) {
    var o = i.optimistic[c.optimisticOrder[n]];
    if (void 0 !== o) {
...
function updatePersist(e, t) {
  if (!f && c.storage) {
    c.persist.add(function serializeKeys(e, t) {
...
function readRecord(e, t) {
  ...
  return getNode(c.records, e, t);
}

function readLink(e, t) {
  updateDependencies(e, t);
  return getNode(c.links, e, t);
}

function writeRecord(e, t, r) {
  updateDependencies(e, t);
  updatePersist(e, t);
  setNode(c.records, e, t, r);
}
...
function writeField(e, t, r, i) {
  ...
  var l = r.__typename;
  if ("production" !== process.env.NODE_ENV) {
    if (i && !e.store.keys[r.__typename] && null === c && "string" == typeof l && !v.test(l)) {
...
Store.prototype.keyOfEntity = function keyOfEntity(e) {
  if (p.current && e === p.current.parent) {
    return p.current.parentKey;
  }
  ...
  return function(i) {
    ...
    function updateCacheWithResult(e, t) {
      ...
      if ("mutation" === r.kind) {
        ...
        reserveLayer(s.data, r.key);
      }
      var o;
      if (e.data) {
        collectPendingOperations(t, write(s, r, e.data, e.error, a).dependencies);
        var u = query(s, r, e.data, e.error, a);
        e.data = u.data;
        if ("query" === r.kind) {
    ...
    return function(e) {
      ...
      var h = t.share(n(t.map(prepareForwardedOperation)(t.merge([ v, y ]))));
